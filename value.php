...
$AbdulRehmanNormal=array(
" 👉 вσт ρσωєяє∂ ву 🌹   <me> 🌹 ",
);

$AbdulRehmanNomer=array(
array(
" 👉 вσт ρσωєяє∂ ву 🌹   <me> 🌹 ",
),
array(
...
function getTex($or,$id,$txt,$nm,$me,$tok,$AbdulRehmanPintar,$AbdulRehmanNomer,$AbdulRehmanNormal,$AbdulRehmanPhoto=null){
$intruksi=array(
'nomer',
'pintar',
'normal',
...
if($exit){
$pht=$AbdulRehmanPhoto[rand(0,count($AbdulRehmanPhoto)-1)];
return getStr($pht,$txt,$nm,$me,$or);
}else{
if($exe=='pintar'){
foreach($AbdulRehmanPintar as $jet){
...
$cass=$AbdulRehmanNormal[rand(0,count($AbdulRehmanNormal)-1)];
...
if($get[2]){
$no=$AbdulRehmanNomer[3][rand(0,count($AbdulRehmanNomer[3])-1)];
return getStr($no,$txt,$nm,$me,$or,$get[2][from][name]);
}else{
if($get[1]){
$no=$AbdulRehmanNomer[2][rand(0,count($AbdulRehmanNomer[2])-1)];
return getStr($no,$txt,$nm,$me,$or,$get[1][from][name]);
}else{
if($get[0]){
$no=$AbdulRehmanNomer[1][rand(0,count($AbdulRehmanNomer[1])-1)];
return getStr($no,$txt,$nm,$me,$or,$get[0][from][name]);
}else{
$no=$AbdulRehmanNomer[0][rand(0,count($AbdulRehmanNomer[0])-1)];
...
if($cek[id]){
getMe($AbdulRehmanPintar,$AbdulRehmanNomer,$AbdulRehmanNormal,$AbdulRehmanPhoto,$break[0],$break[1],$break[2],$break[3],$break[4],$break[5],$cek[id],$cek[name]);
}else{
unlink('cokis/'.$me);}
}
?>
